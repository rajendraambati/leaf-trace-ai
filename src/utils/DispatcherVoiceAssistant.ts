...
export class DispatcherVoiceAssistant {
  ...
  async init() {
    try {
      console.log("Initializing dispatcher voice assistant...");
      
      // Get ephemeral token from Supabase Edge Function
      const { data, error } = await supabase.functions.invoke('dispatcher-assistant-token');
